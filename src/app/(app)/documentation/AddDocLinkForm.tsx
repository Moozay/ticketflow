...
export default function AddDocLinkForm() {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [isPending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState({ title: '', url: '', section: '', description: '', order: '0' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        setOpen(false)
        setForm({ title: '', url: '', section: '', description: '', order: '0' })
        router.refresh()
      } catch {
        ...
        >
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', width: '100%', maxWidth: '480px', padding: '28px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>Add Documentation Link</h2>
